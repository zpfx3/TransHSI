...
if __name__ == '__main__':
    ...
    data_path = 'dataset/PaviaU/'
    dataset = cfg['data']["dataset"]
    num_components = cfg['data']['num_components']
    if dataset == 'PU':
        # 直接读取pca数据
        data = sio.loadmat(os.path.join(data_path, 'PaviaUHSI.mat'))['data']

        # #读取ENVI数据
        # import spectral
        # data = spectral.open_image(os.path.join(data_path, 'xiongan_1m_MNFTexture.hdr'))
        # data = data[:,:,:15]
        print(data.shape)
        # import sys
        # sys.exit(0)
        ytrainRaw = sio.loadmat(os.path.join(data_path, 'PaviaUTrainGT.mat'))['data']
        ytrainRaw = ytrainRaw.astype('uint8')
        ytestRaw = sio.loadmat(os.path.join(data_path, 'PaviaUTestGT.mat'))['data']
        ytestRaw = ytestRaw.astype('uint8')
    else:
        print("NO DATASET")
        exit()
    print("load {} original image successfully".format(dataset))
    ...
    num_class = len(np.unique(ytrainRaw)) - 1

    # create patches
    x_train, y_train, train_index = creat_PP(cfg, data, ytrainRaw)
    x_test,  y_test,  test_index = creat_PP(cfg, data, ytestRaw)
    ...
    test_positions = np.zeros(ytestRaw.shape)
